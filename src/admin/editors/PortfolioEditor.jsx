...
export default function PortfolioEditor() {
  const { content, loading, namespace, saveContent, publishContent, undoLastSave, fetchHistory, restoreVersion } = useContent('portfolio')
  ...
  const setItem = (id, key, val) => set('items', form.items.map(i => i.id === id ? { ...i, [key]: val } : i))
  const addItem = () => set('items', [...(form.items || []), {
    id: String(Date.now()), title: 'New Project', category: '', description: '', image: '', link: ''
  }])
  ...
  const handleSave = async () => {
    ...
    else toast.success('Portfolio section saved!')
    setSaving(false)
  }
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      else toast.success(namespace === 'review_draft' ? 'Published to demo site!' : 'Portfolio section published to live site!')
    } catch (err) {
      console.error('[PortfolioEditor] publish error:', err)
  ...
  return (
    <SectionEditor
      title="Portfolio"
      description="Case studies and past projects"
      form={form}
              ...
              <div className="flex items-start gap-3">
                <div className="flex-1 grid sm:grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <input type="text" value={item.category || ''} onChange={e => setItem(item.id, 'category', e.target.value)}
                  ...
                  <div className="sm:col-span-2">
                    <label className="block text-xs font-medium text-gray-500 mb-1">Live Link (optional)</label>
                    <input type="text" value={item.link || ''} onChange={e => setItem(item.id, 'link', e.target.value)}
                      placeholder="https://yourproject.com"
                      className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                  </div>
                  <div className="sm:col-span-2">
                    <label className="block text-xs font-medium text-gray-500 mb-1">Project Image</label>
                    <ImageUpload
                      value={item.image}
                      onChange={(url) => setItem(item.id, 'image', url)}
                      folder="portfolio"
                    />
                  </div>
                </div>
