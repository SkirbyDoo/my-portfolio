// CityHoopsEditor — CLIENT-SPECIFIC custom editor (NOT a core dashboard file).
// Edits the `casestudy_cityhoops` content section that powers the hard-coded
// case study page at /work/cityhoops (src/pages/CityHoops.jsx).
//
...
// ── string-list editor (teams, built bullets) ────────────────────────────────
...
export default function CityHoopsEditor() {
  const namespace = useContentNamespace()
  const { content, loading, saveContent, publishContent, undoLastSave } = useContent('casestudy_cityhoops')
  ...
  const handleSave = async () => {
    ...
    else toast.success('CityHoops case study saved!')
    setSaving(false)
  }
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      else toast.success('CityHoops case study published to live site!')
    } catch (err) {
      console.error('[CityHoopsEditor] publish error:', err)
    ...
    <div className="flex flex-col min-h-[420px]">
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-5 py-2 sm:py-3 bg-white border-b border-gray-200 shrink-0 sticky top-0 z-10">
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-gray-800 truncate">CityHoops Case Study</p>
          <p className="hidden sm:block text-xs text-gray-400 truncate">Custom page at /work/cityhoops — edit copy, stats &amp; images</p>
        </div>
      ...
      <div className="p-3 sm:p-5 space-y-5 bg-gray-50 flex-1 overflow-y-auto">
        ...
        <GroupCard title="Hero">
          ...
          <Field label="Live Site URL" hint="Leave blank to hide the “Visit Live Site” button.">
            <Text value={form.liveUrl} onChange={v => set('liveUrl', v)} placeholder="https://cityhoopsleague.com" />
          </Field>
        </GroupCard>
        ...
        <GroupCard title="Overview">
          ...
          <Field label="Team chips">
            <StringList items={form.teams} onChange={v => set('teams', v)} addLabel="Add team" placeholder="Team name" />
          </Field>
        </GroupCard>
        ...
        <GroupCard title="The Challenge (Before / After: Home)">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <ImageUpload value={images.oldHome} onChange={url => setImage('oldHome', url)} folder="cityhoops" label="Old homepage (Before)" />
            <ImageUpload value={images.newHome} onChange={url => setImage('newHome', url)} folder="cityhoops" label="New homepage (After)" />
          </div>
        </GroupCard>
        ...
        <GroupCard title="What I Built">
          ...
          <div className="pt-2 border-t border-gray-100 space-y-4">
            <Field label="Schedule sub-heading">
              <Text value={form.scheduleHeading} onChange={v => set('scheduleHeading', v)} />
            </Field>
            <div className="grid sm:grid-cols-2 gap-4">
              <ImageUpload value={images.oldSchedule} onChange={url => setImage('oldSchedule', url)} folder="cityhoops" label="Old schedule (Before)" />
              <ImageUpload value={images.newSchedule} onChange={url => setImage('newSchedule', url)} folder="cityhoops" label="New schedule (After)" />
            </div>
          </div>

          <div className="pt-2 border-t border-gray-100 space-y-4">
            <Field label="Stats sub-heading">
              <Text value={form.statsHeading} onChange={v => set('statsHeading', v)} />
            </Field>
            <Field label="Stats description">
              <Area value={form.statsBody} onChange={v => set('statsBody', v)} rows={3} />
            </Field>
            <ImageUpload value={images.newStats} onChange={url => setImage('newStats', url)} folder="cityhoops" label="Stats leaderboard" />
          </div>
        </GroupCard>
