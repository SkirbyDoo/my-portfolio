// QuickQuoteEditor — CLIENT-SPECIFIC custom editor (NOT a core dashboard file).
// Edits the `casestudy_quickquote` content section that powers the hard-coded
// case study page at /work/quick-quote (src/pages/QuickQuote.jsx).
//
...
// ── string-list editor (input chips, built bullets) ───────────────────────────
...
export default function QuickQuoteEditor() {
  const namespace = useContentNamespace()
  const { content, loading, saveContent, publishContent, undoLastSave } = useContent('casestudy_quickquote')
  ...
  const handleSave = async () => {
    ...
    else toast.success('Quick Quote case study saved!')
    setSaving(false)
  }
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      else toast.success('Quick Quote case study published to live site!')
    } catch (err) {
      console.error('[QuickQuoteEditor] publish error:', err)
    ...
    <div className="flex flex-col min-h-[420px]">
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-5 py-2 sm:py-3 bg-white border-b border-gray-200 shrink-0 sticky top-0 z-10">
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-gray-800 truncate">Quick Quote Case Study</p>
          <p className="hidden sm:block text-xs text-gray-400 truncate">Custom page at /work/quick-quote — edit copy, stats &amp; images</p>
        </div>

        <a
          href="/work/quick-quote"
          target="_blank"
      ...
      <div className="p-3 sm:p-5 space-y-5 bg-gray-50 flex-1 overflow-y-auto">
        ...
        <GroupCard title="Hero">
          ...
          <Field label="Live Site URL" hint="Leave blank to hide the “Try the Calculator” button.">
        ...
        <GroupCard title="Overview">
          ...
          <Field label="Input chips" hint="The fields a customer fills in — shown as little pills.">
            <StringList items={form.inputs} onChange={v => set('inputs', v)} addLabel="Add input" placeholder="e.g. Quantity" />
          </Field>
        </GroupCard>
